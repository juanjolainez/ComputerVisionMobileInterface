...
%min es el primer elemento que tiene más area arriba que abajo y es
%candidato a ser la muñeca
%Absmin es el candidato con el ancho más fino

for j=min:limite
    if (nums(j,1) < nums(absmin,1) && nums(j,3) > nums(j,4)) 
        absmin = j;
        min = j;
    elseif (abs(nums(j,1)-nums(absmin,1)) <= 10) %si son similares, ajustar con tamaño imagen     
